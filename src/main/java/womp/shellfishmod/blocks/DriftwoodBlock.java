...
public class DriftwoodBlock extends Block {
    ...
    public BlockState getStateForNeighborUpdate(BlockState state, WorldView world, ScheduledTickView view, BlockPos pos, Direction direction, BlockPos neighborPos, BlockState neighborState, Random random) {
        ...
        if (world.isWater(pos.down()) && !world.getBlockState(pos.down()).isSideSolidFullSquare(world, pos.down(), Direction.UP)) return super.getStateForNeighborUpdate(state, world, view, pos, direction, neighborPos, neighborState, random).with(ON_WATER, true);
        return super.getStateForNeighborUpdate(state, world, view, pos, direction, neighborPos, neighborState, random).with(ON_WATER, false);
    }
