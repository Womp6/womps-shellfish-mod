...
public class CrayfishModel extends SinglePartEntityModel<CrayfishEntity> {
	...
	@Override
	public void setAngles(CrayfishEntity entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
		this.getPart().traverse().forEach(ModelPart::resetTransform);
		if (!entity.isTouchingWater()) {
			this.animateMovement(MoreShellfishAnimations.CRAYFISH_WALK, limbSwing, limbSwingAmount, 12, 15f);
		}
		this.updateAnimation(entity.moveAnimationState, MoreShellfishAnimations.CRAYFISH_WALK, ageInTicks, 2f);
		this.updateAnimation(entity.idleAnimationState, MoreShellfishAnimations.CRAYFISH_IDLE, ageInTicks, 1f);
	}

	@Override
	public void render(MatrixStack matrices, VertexConsumer vertexConsumer, int light, int overlay, int color) {
		crayfish.render(matrices, vertexConsumer, light, overlay, color);
	}
