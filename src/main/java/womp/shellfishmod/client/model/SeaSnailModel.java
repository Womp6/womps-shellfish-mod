...
public class SeaSnailModel extends SinglePartEntityModel<SeaSnailEntity> {
	...
	@Override
	public void setAngles(SeaSnailEntity entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
		this.getPart().traverse().forEach(ModelPart::resetTransform);
		this.updateAnimation(entity.moveAnimationState, ShellfishAnimations.SNAIL_MOVE, ageInTicks, 1f);
		this.updateAnimation(entity.idleAnimationState, ShellfishAnimations.SNAIL_HIDE, ageInTicks, 1f);
	}

	@Override
	public void render(MatrixStack matrices, VertexConsumer vertexConsumer, int light, int overlay, int color) {
		sea_snail.render(matrices, vertexConsumer, light, overlay, color);
	}
