...
public class SeaUrchinModel extends HierarchicalModel<SeaUrchinEntity> {
    ...
    @Override
    public void setupAnim(SeaUrchinEntity entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
        this.root().getAllParts().forEach(ModelPart::resetPose);
        this.animate(entity.moveAnimationState, ShellfishAnimations.SEA_URCHIN_WALK, ageInTicks, 1f);
        this.animate(entity.idleAnimationState, ShellfishAnimations.SEA_URCHIN_IDLE, ageInTicks, 1f);
    }

    @Override
    public void renderToBuffer(PoseStack matrices, VertexConsumer vertexConsumer, int light, int overlay, int color) {
        sea_urchin.render(matrices, vertexConsumer, light, overlay, color);
    }
