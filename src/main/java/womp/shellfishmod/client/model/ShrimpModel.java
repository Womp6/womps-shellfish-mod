...
public class ShrimpModel extends HierarchicalModel<ShrimpEntity> {
    ...
    public void setupAnim(ShrimpEntity entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
        ...
        if (!entity.isInWater()) {
            this.animateWalk(ShellfishAnimations.SHRIMP_WALK, limbSwing, limbSwingAmount, 12, 15f);
        }
        this.animate(entity.idleAnimationState, ShellfishAnimations.SHRIMP_IDLE, ageInTicks, 1f);
        this.animate(entity.swimAnimationState, ShellfishAnimations.SHRIMP_SWIM, ageInTicks, 2f);
    }
