...
public class ShellfishCrayfish {
    ...
    public static void register() {
        ...
        for (String[] type : crayfish) {
            ItemProperties.register(ShellfishItems.CRAYFISH.get(), ResourceLocation.parse(type[1]), (stack, world, entity, seed) -> {
                if (stack.getComponents().has(DataComponents.CUSTOM_NAME) && type[0].equals(stack.get(DataComponents.CUSTOM_NAME).getString())) {
                    return 1.0F;
                }
                return 0.0F;
            });
