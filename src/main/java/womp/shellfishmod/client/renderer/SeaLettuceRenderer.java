...
public class SeaLettuceRenderer implements BlockEntityRenderer<SeaLettuceBlockEntity> {
    ...
    private final Identifier texture = new Identifier("shellfish", "textures/block/sea_lettuce.png");
    ...
    @Override
    public void render(SeaLettuceBlockEntity blockEntity, float tickDelta, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light, int overlay) {
        ...
        if(ShellfishConfig.getShellfishGraphics() == 2) {
            ...
            seaLettuceModel.render(blockEntity, matrices, vertexConsumers.getBuffer(RenderLayer.getEntityCutoutNoCull(getTexture(blockEntity))), light, overlay);
