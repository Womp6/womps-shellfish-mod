...
public class WaterLettuceRenderer implements BlockEntityRenderer<WaterLettuceBlockEntity> {
    ...
    @Override
    public void render(WaterLettuceBlockEntity blockEntity, float tickDelta, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light, int overlay, Vec3d vec) {
        ...
        if (ShellfishConfig.getShellfishGraphics() >= 1) {
            ...
            lettuceModel.render(matrices, vertexConsumers.getBuffer(RenderLayer.getEntityCutout(getTexture(blockEntity))), light, overlay, vec);
