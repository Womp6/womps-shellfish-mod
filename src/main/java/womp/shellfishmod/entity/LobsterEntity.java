...
public class LobsterEntity extends ShellfishEntity<LobsterEntity.Variant> implements Hungry, EggLaying {
    ...
    public LobsterEntity(EntityType<? extends LobsterEntity> entityType, Level world) {
        ...
        depth = 50;
        hostileSound = ShellfishSounds.LOBSTER_ATTACK.get();
    }
    ...
    protected void registerGoals() {
        this.goalSelector.addGoal(0, new ShellfishLayEggGoal(this, 1, ShellfishSounds.LOBSTER_LAYS_EGGS.get(), ShellfishBlocks.LOBSTER_EGGS_BLOCK.get()));
    ...
    public AgeableMob getBreedOffspring(ServerLevel world, AgeableMob entity) {
        LobsterEntity child;
        if((child = ShellfishEntities.LOBSTER.get().create(world, EntitySpawnReason.BREEDING)) != null && entity instanceof LobsterEntity mate) {
    ...
    protected void usePlayerItem(Player player, InteractionHand hand, ItemStack stack) {
        if (stack.is(Items.TROPICAL_FISH_BUCKET)) {
            ...
            player.setItemInHand(hand, new ItemStack(Items.WATER_BUCKET));
        } else if (stack.is(ShellfishItems.CRAB_BUCKET.get())) {
            player.setItemInHand(hand, new ItemStack(Items.WATER_BUCKET));
        } else if (stack.is(ShellfishItems.CRAYFISH_BUCKET.get())) {
            player.setItemInHand(hand, new ItemStack(Items.WATER_BUCKET));
    ...
    public ItemStack getBucketItemStack() {
        return new ItemStack(ShellfishItems.LOBSTER_BUCKET.get());
    }

    @Override
    @Nullable
    protected SoundEvent getDeathSound() {
        return ShellfishSounds.LOBSTER_DEATH.get();
    }

    @Override
    @Nullable
    protected SoundEvent getHurtSound(DamageSource source) {
        return ShellfishSounds.LOBSTER_HURT.get();
    }
