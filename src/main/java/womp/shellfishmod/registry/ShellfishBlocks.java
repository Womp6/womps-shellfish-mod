...
public class ShellfishBlocks {
    ...
    private static Block registerCS(String name, boolean clay) {
        return register(name, clay ? new Block(FabricBlockSettings.copy(Blocks.CLAY).instrument(Instrument.FLUTE).strength(0.6F).sounds(BlockSoundGroup.GRAVEL)) : new FallingBlock(FabricBlockSettings.copy(Blocks.SAND).instrument(Instrument.SNARE).strength(0.5F).sounds(BlockSoundGroup.SAND)), name);
    }
