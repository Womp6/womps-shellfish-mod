...
public class ShellfishBlocks {
    ...
    //PLANTS
    public static final Block ROCKWEED = register("rockweed", new RockWeedBlock(settingsPlant(Blocks.SEAGRASS, false)), "rockweed");
	public static final Block TALL_ROCKWEED = registerNoBI("tall_rockweed", new TallRockWeedBlock(settingsPlant(Blocks.TALL_SEAGRASS, true)));
    private static final Block water_lettuce_data = new WaterLettuceBlock(FabricBlockSettings.copy(Blocks.LILY_PAD).breakInstantly().pistonBehavior(PistonBehavior.DESTROY).sounds((BlockSoundGroup.BIG_DRIPLEAF)));
    public static final Block WATER_LETTUCE = registerCBI("water_lettuce", water_lettuce_data, "water_lettuce", new PlaceableOnWaterItem(water_lettuce_data, new Item.Settings()));
    ...
    private static final Block sea_lettuce_data = new SeaLettuceBlock(settingsPlant(ROCKWEED, false));
    public static final Block SEA_LETTUCE = registerCBI("sea_lettuce", sea_lettuce_data, "sea_lettuce", new BlockItem(sea_lettuce_data, new FabricItemSettings().food((new FoodComponent.Builder()).hunger(1).saturationModifier((float) 0.1).build())));
    ...
    public static final Block driftwood_data = new DriftwoodBlock(FabricBlockSettings.create().strength(0.2f).sounds(BlockSoundGroup.WOOD).burnable().pistonBehavior(PistonBehavior.DESTROY));
    public static final Block DRIFTWOOD = registerCBI("driftwood", driftwood_data, "driftwood", new PlaceableOnWaterItem(driftwood_data, new Item.Settings()));

    //SHELLFISH TRAPS
    public static final Block SHELLFISH_TRAP_BLOCK = register("shellfish_trap_block", new ShellfishTrapBlock(FabricBlockSettings.copy(Blocks.IRON_BLOCK).strength(5.0f, 6.0f).sounds(BlockSoundGroup.CHAIN).nonOpaque()), "shellfish_trap");
    public static final Block REINFORCED_TRAP = register("reinforced_trap", new ReinforcedTrapBlock(FabricBlockSettings.copy(Blocks.DIAMOND_BLOCK).strength(5.5f, 6.5f).sounds(BlockSoundGroup.CHAIN).nonOpaque()), "reinforced_trap");
    ...
    public static final Block BARREL_NO_POI = registerNoBI("barrel", new BarrelBlock(FabricBlockSettings.copy(Blocks.BARREL)));

    private static Block registerLandWaterPlant(String name, boolean waterOnly, Block tallPlantBlock, boolean grass) {
        return register(name, grass ? new ShellfishLandPlantBlock(AbstractBlock.Settings.copy(Blocks.SHORT_GRASS), waterOnly, tallPlantBlock) : new WaterFlowerBlock(AbstractBlock.Settings.copy(Blocks.DANDELION), tallPlantBlock), name);
    }

    private static Block registerTallPlant(String name, boolean waterOnly, boolean grass) {
        Block block = new ShellfishLandTallBlock(AbstractBlock.Settings.copy(grass ? Blocks.SHORT_GRASS : Blocks.DANDELION), waterOnly);
        return registerCBI(name, block, name, new TallBlockItem(block, new Item.Settings()));
    }

    private static Block registerPlant(String name, VoxelShape shape) {
        return register(name, new ShellfishPlantBlock(settingsPlant(Blocks.SEAGRASS, true), shape, PlaceType.MUD_SAND), name);
    }

    private static Block.Settings settingsPlant(Block block, boolean offset) {
        Block.Settings settings = FabricBlockSettings.copy(block).breakInstantly().noCollision().replaceable().sounds((BlockSoundGroup.WET_GRASS)).pistonBehavior(PistonBehavior.DESTROY);
    ...
    private static Block registerDeadBlock(String name, VoxelShape shape, boolean solid) {
        AbstractBlock.Settings sets = FabricBlockSettings.copy(Blocks.TURTLE_EGG).breakInstantly().sounds((BlockSoundGroup.CALCITE)).pistonBehavior(PistonBehavior.DESTROY);
    ...
    private static Block registerCSM(String name, int type) {
        return register(name, type == 1 ? new Block(FabricBlockSettings.copy(Blocks.CLAY).instrument(Instrument.FLUTE).strength(0.6F).sounds(BlockSoundGroup.GRAVEL)) : type == 2 ? new ColoredFallingBlock(new ColorCode(14406560), FabricBlockSettings.copy(Blocks.SAND).instrument(Instrument.SNARE).strength(0.5F).sounds(BlockSoundGroup.SAND)) : new MudBlock(AbstractBlock.Settings.copy(Blocks.MUD)), name);
    }

    private static Block registerShell(String name, VoxelShape shape) {
        return register(name, new ShellBlock(FabricBlockSettings.copy(Blocks.TURTLE_EGG).breakInstantly().sounds(BlockSoundGroup.CALCITE).pistonBehavior(PistonBehavior.DESTROY), shape), name);
    }

    private static Block registerEggsBlock(String name, EntityType<? extends ShellfishEntity> entity, SoundEvent hatchSound) {
        return register(name + "_eggs_block", new EggsBlock(AbstractBlock.Settings.create().mapColor(MapColor.WATER_BLUE).breakInstantly().noCollision().sounds(BlockSoundGroup.FROGSPAWN).pistonBehavior(PistonBehavior.DESTROY).nonOpaque(), entity, hatchSound), name + "_eggs");
    }

    private static Block registerCBI(String name, Block factory, String itemName, Item factory2) {
        Registry.register(Registries.ITEM, new Identifier("shellfish", itemName), factory2);
        return Registry.register(Registries.BLOCK, new Identifier("shellfish", name), factory);
    }

    private static Block register(String name, Block factory, String itemName) {
        return registerCBI(name, factory, itemName, new BlockItem(factory, new Item.Settings()));
    }
    ...
    private static <T extends BlockEntity> BlockEntityType<T> registerBE(String name, FabricBlockEntityTypeBuilder.Factory<T> factory, Block block) {
        return Registry.register(Registries.BLOCK_ENTITY_TYPE, new Identifier("shellfish", name), FabricBlockEntityTypeBuilder.create(factory, block).build(null));
    }
    
    public static void register() {
        FuelRegistry.INSTANCE.add(DRIFTWOOD, 200);
