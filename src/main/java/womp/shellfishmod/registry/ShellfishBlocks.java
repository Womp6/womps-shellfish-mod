...
public class ShellfishBlocks {
    ...
    private static final Block water_lettuce_data = new WaterLettuceBlock(AbstractBlock.Settings.copy(Blocks.LILY_PAD).breakInstantly().pistonBehavior(PistonBehavior.DESTROY).sounds((BlockSoundGroup.BIG_DRIPLEAF)));
    ...
    public static final Block SEA_LETTUCE = registerCBI("sea_lettuce", sea_lettuce_data, "sea_lettuce", new BlockItem(sea_lettuce_data, new Item.Settings().food((new FoodComponent.Builder()).nutrition(1).saturationModifier((float) 0.1).build())));
    ...
    public static final Block driftwood_data = new DriftwoodBlock(AbstractBlock.Settings.create().strength(0.2f).sounds(BlockSoundGroup.WOOD).burnable().pistonBehavior(PistonBehavior.DESTROY));
    ...
    //SHELLFISH TRAPS
    public static final Block SHELLFISH_TRAP_BLOCK = register("shellfish_trap_block", new ShellfishTrapBlock(AbstractBlock.Settings.copy(Blocks.IRON_BLOCK).strength(5.0f, 6.0f).sounds(BlockSoundGroup.CHAIN).nonOpaque()), "shellfish_trap");
    public static final Block REINFORCED_TRAP = register("reinforced_trap", new ReinforcedTrapBlock(AbstractBlock.Settings.copy(Blocks.DIAMOND_BLOCK).strength(5.5f, 6.5f).sounds(BlockSoundGroup.CHAIN).nonOpaque()), "reinforced_trap");
    ...
    public static final Block BARREL_NO_POI = registerNoBI("barrel", new BarrelBlock(AbstractBlock.Settings.copy(Blocks.BARREL)));
    ...
    private static Block registerDeadBlock(String name, VoxelShape shape, boolean solid) {
        AbstractBlock.Settings sets = AbstractBlock.Settings.copy(Blocks.TURTLE_EGG).breakInstantly().sounds((BlockSoundGroup.CALCITE)).pistonBehavior(PistonBehavior.DESTROY);
    ...
    private static Block registerCSM(String name, int type) {
        return register(name, type == 1 ? new Block(AbstractBlock.Settings.copy(Blocks.CLAY).instrument(Instrument.FLUTE).strength(0.6F).sounds(BlockSoundGroup.GRAVEL)) : type == 2 ? new ColoredFallingBlock(new ColorCode(14406560), AbstractBlock.Settings.copy(Blocks.SAND).instrument(Instrument.SNARE).strength(0.5F).sounds(BlockSoundGroup.SAND)) : new MudBlock(AbstractBlock.Settings.copy(Blocks.MUD)), name);
    }

    private static Block registerShell(String name, VoxelShape shape) {
        return register(name, new ShellBlock(AbstractBlock.Settings.copy(Blocks.TURTLE_EGG).breakInstantly().sounds(BlockSoundGroup.CALCITE).pistonBehavior(PistonBehavior.DESTROY), shape), name);
    }
    ...
    private static <T extends BlockEntity> BlockEntityType<T> registerBE(String name, BlockEntityFactory<T> factory, Block block) {
        return Registry.register(Registries.BLOCK_ENTITY_TYPE, new Identifier("shellfish", name), BlockEntityType.Builder.create(factory, block).build(null));
    }
