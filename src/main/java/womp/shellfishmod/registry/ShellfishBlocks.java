...
public class ShellfishBlocks {
    ...
    public static final Block REINFORCED_TRAP = registerDurability("reinforced_trap", new ReinforcedTrapBlock(AbstractBlock.Settings.copy(Blocks.DIAMOND_BLOCK).strength(5.5f, 6.5f).sounds(BlockSoundGroup.CHAIN).nonOpaque().registryKey(ShellfishUtil.createKey("reinforced_trap", RegistryKeys.BLOCK))), "reinforced_trap", 300);
    ...
    private static Block registerDurability(String name, Block factory, String itemName, int max) {
        return registerCBI(name, factory, itemName, new TrapBlockItem(factory, new Item.Settings().registryKey(ShellfishUtil.createKey(itemName, RegistryKeys.ITEM)), max));
    }
