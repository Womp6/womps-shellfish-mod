...
public class ShellfishFeatures {
    ...
    public static final BlockStateProviderType<ConditionalBlockStateProvider> CONDITIONAL_STATE_PROVIDER = Registry.register(Registries.BLOCK_STATE_PROVIDER_TYPE, Identifier.of("shellfish", "conditional_state_provider"), new BlockStateProviderType<ConditionalBlockStateProvider>(ConditionalBlockStateProvider.CODEC));
    ...
    private static <T extends FeatureConfig> Feature<T> register(String name, Feature<T> feature) {
        return Registry.register(Registries.FEATURE, Identifier.of("shellfish", name), feature);
    }
