...
public class ShellfishItems {
    ...
    private static Item registerBucket(String name, EntityType<? extends MobEntity> type) {
        return registerItem(name, new ShellfishBucketItem(() -> type, Fluids.WATER, Items.BUCKET, true, new Item.Settings().maxCount(1).recipeRemainder(Items.BUCKET).registryKey(ShellfishUtil.createKey(name, RegistryKeys.ITEM))));
    }

    private static Item registerEgg(String name, EntityType<? extends MobEntity> type, int primaryColor, int secondaryColor) {
        return registerItem(name, new SpawnEggItem(type, primaryColor, secondaryColor, new Item.Settings().registryKey(ShellfishUtil.createKey(name, RegistryKeys.ITEM))));
    }
