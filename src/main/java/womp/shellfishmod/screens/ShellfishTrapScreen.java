...
public class ShellfishTrapScreen extends AbstractContainerScreen<ShellfishTrapScreenHandler> {
    ...
    public static final ResourceLocation TEXTURE = new ResourceLocation("shellfish", "textures/gui/shellfish_trap.png");
    ...
    public void renderProgressArrow(GuiGraphics context, int x, int y) {
        if (menu.isTrapping()) {
            context.blit(TEXTURE, x + 32, y + 38, 176, 0, menu.getScaledProgress(), 10);
        }
    }
    ...
    public void renderDurabilityBar(GuiGraphics context, int x, int y) {
        if (menu.getDurability() > menu.getMaxDurability() / 2) {
            context.blit(TEXTURE, x + 12, y + 16, 177, 10, menu.getScaledDurability(), 8);
        } else if (menu.getDurability() > menu.getMaxDurability() / 5) {
            context.blit(TEXTURE, x + 12, y + 16, 177, 18, menu.getScaledDurability(), 8);
        } else {
            context.blit(TEXTURE, x + 12, y + 16, 177, 26, menu.getScaledDurability(), 8);
        }

        if (menu.getDurability() != 0) {
            context.blit(TEXTURE, x + 11, y + 16, 176, 10, 1, 8);
        }
        if (menu.getDurability() == menu.getMaxDurability()) {
            context.blit(TEXTURE, x + 52, y + 16, 217, 10, 1, 8);
        }
    }
